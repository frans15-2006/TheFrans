...
function sendOrderToServer(orderData) {
  try {
    const existing = JSON.parse(localStorage.getItem('mmk_orders') || '[]');
    const newOrder = { ...orderData, id: existing.length + 1, date: new Date().toISOString() };
    existing.unshift(newOrder);
    localStorage.setItem('mmk_orders', JSON.stringify(existing));
